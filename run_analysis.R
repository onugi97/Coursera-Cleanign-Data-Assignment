...
library(reshape2)
output = melt(mergedData, id=c("subject","activity"))
output2 = dcast(output, subject + activity ~ variable, mean); dim(temp2)
# Output the tidy data set to file output.txt
write.table(x=output2, "tidyData.txt", row.names=FALSE)
